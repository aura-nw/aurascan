...
export class CosmosSnapClient implements WalletClient {
  ...
  async signDirect(chainId: string, signer: string, signDoc: DirectSignDoc): Promise<DirectSignResponse> {
  ...
  async signArbitrary(chainId: string, signer: string, data: string): Promise<StdSignature> {
    return (await signArbitrary(chainId, signer, data)) as unknown as StdSignature;
  }
  ...
  async addChain(chainRecord: ChainRecord): Promise<void> {
    ...
    await experimentalSuggestChain(chainInfo, null);
  }
}
