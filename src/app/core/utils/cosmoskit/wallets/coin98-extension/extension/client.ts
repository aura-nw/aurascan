...
export class Coin98Client implements WalletClient {
  ...
  getOfflineSignerDirect(chainId: string) {
    return {
      ...
      signDirect: (signerAddress: string, signDoc: SignDoc) => this.signDirect(chainId, signerAddress, signDoc),
    };
  }
  ...
  async addChain(chainInfo: ChainRecord) {
    ...
    if (chainInfo.preferredEndpoints?.rest?.[0]) {
      (suggestChain.rest as string | ExtendedHttpEndpoint) = chainInfo.preferredEndpoints?.rest?.[0];
    }
  ...
  async signDirect(chainId: string, signer: string, signDoc: DirectSignDoc, signOptions?: SignOptions) {
    ...
    const accountNumber = Long.fromValue(resp.signed.accountNumber).toString();
    return {
      ...resp,
      signed: {
        ...resp.signed,
