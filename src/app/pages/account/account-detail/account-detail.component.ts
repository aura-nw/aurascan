...
import { COSMOS_ACCOUNT_MESSAGE_TYPE, ACCOUNT_WALLET_COLOR, COSMOS_WARNING_MESSAGE, EVM_WARNING_MESSAGE, EVM_ACCOUNT_MESSAGE_TYPE } from '../../../core/constants/account.constant';
      ...
      if (params?.address) {
        ...
        this.userService.getAccountInfoOfAddress(payload).subscribe({
          next: (data: { account?: {type?: any; sequence?: number; pubkey?: object}[] } = {}) => {
            ...
            if(type === EVM_ACCOUNT_MESSAGE_TYPE && !Object.keys(pubkey)?.length){
              this.accountType = 'evm';
              this.tooltipCosmosText = COSMOS_WARNING_MESSAGE;
              return;
            }else if (type === COSMOS_ACCOUNT_MESSAGE_TYPE && Object.keys(pubkey)?.length) {
