    ...
    this.chart.subscribeCrosshairMove((param) => {
      if (param.point === undefined || !param.time || param.point.x < 0 || param.point.x > container.clientWidth || param.point.y < 0 || param.point.y > container.clientHeight) {
        ...
        toolTip.innerHTML = '' +
          ...
          '<div class="floating-tooltip__body"><div style="font-size: 14px; margin: 4px 0;">' + Math.round(100 * price) / 100 + '</div><div>' + dateStr + '' +'</div></div>';
        ...
        var coordinateY = coordinate - this.toolTipHeight - this.toolTipMargin > 0 ? coordinate - this.toolTipHeight - this.toolTipMargin :
          Math.max(0, Math.min(container.clientHeight - this.toolTipHeight - this.toolTipMargin, coordinate + this.toolTipMargin));
