...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
        ...
        if (this.searchValue) {
            ...
            if (regexRule.test(this.searchValue)) {
                if (this.searchValue.length === LENGTH_CHARACTER.TRANSACTION) {
                    ...
                    if (await this.checkIsContractDetailExisted(this.searchValue)) {
                        await this.router.navigate(['contracts', this.searchValue]);
                    } else {
    ...
    async checkIsContractDetailExisted(address) {
        const req = await this.contractService.getContractDetail(address).toPromise();
        if (req.data) {
            return true;
        } else {
            return false;
